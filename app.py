...
marriage_label = st.selectbox(
    "Marriage status",
    ["Never married", "Previously married (including separated)", "Married"]
)
marriage_map = {
    "Never married": 1,
    "Previously married (including separated)": 2,
    "Married": 3
}
Marriage_status = marriage_map [marriage_label]

Living_label = st.selectbox(
    "Living status",
    ["Not alone", "Alone"]
)
Living_map = {
...
Living_status = Living_map [Living_label]
...
if st.button("Predict probability"):
    try:
        ...
        st.subheader("Predicted probability of depression")
        st.write(f"{prob:.2%}")
